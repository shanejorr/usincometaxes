...
`usincometaxes` provides the same output as [TAXSIM 32](http://users.nber.org/~taxsim/taxsim32/). 
...
## Mapping `usincometaxes` columns to TAXSIM 32 columns

Results from `usincometaxes` include all output from TAXSIM 32. The table below maps `usincometaxes` 
output column names to TAXSIM 32 output column names.

```{r, echo = FALSE}
package_to_taxsim <- data.frame(
  usincometaxes = unname(usincometaxes:::from_taxsim_cols()),
  TAXSIM = names(usincometaxes:::from_taxsim_cols())
)

knitr::kable(package_to_taxsim)
```

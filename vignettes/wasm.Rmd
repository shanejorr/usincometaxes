...
`usincometaxes` uses a JavaScript / WebAssembly version of TAXSIM 35 to calculate taxes. THe JavaScript / WebAssembly files are part of the package. Therefore, you can calcualte taxes without sending data to the TAXSIM server. The output *should* be the same as the output from the TAXSIM 35 server. But, keep reading for why this might not always be the case.

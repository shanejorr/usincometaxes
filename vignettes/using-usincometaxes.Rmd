...
We call `taxsim_calculate_taxes()` to estimate federal and state income taxes for each tax paying unit. `taxsim_calculate_taxes()` sends the data to the [NBER's TAXSIM 35](http://taxsim.nber.org/taxsim35/) program for calculation and returns the results as a data frame.
...
n_kids <- max(income_and_taxes$num_dependents)

income_and_taxes %>%
  mutate(
    tax_unit_income = primary_wages + spouse_wages,
    num_dependents = factor(num_dependents, levels = as.character(0:n_kids)),
    filing_status = tools::toTitleCase(filing_status)
  ) %>%
  ggplot(aes(tax_unit_income, federal_taxes, color = num_dependents)) +
    geom_point(alpha = .5) +
...
base_family_income <- data.frame(
  ...
  spouse_age = 40,
  num_dependents = 4,
  age_youngest_dependent = 1, 
  age_second_youngest_dependent = 2, 
  age_third_youngest_dependent = 3,
...
family_income <- base_family_income %>%
  bind_rows(
    # make all number of dependent columns 0
    base_family_income %>%
      mutate(across(starts_with('age'), ~0)) %>%
      mutate(num_dependents = 0)
  ) %>%
...
family_income_long <- family_income %>%
  select(primary_wages, num_dependents, federal_taxes, state_taxes) %>%
...
taxes_line_plot(family_income_long, primary_wages, taxes_paid, num_dependents) +
...
taxes_line_plot(family_income_long, primary_wages, post_tax_wages, num_dependents) +
  facet_wrap(vars(jurisdiction)) +
  labs(

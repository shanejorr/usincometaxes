...
family_income <- data.frame(
  ...
  year = c(2015, 2020),
  # for mstat (filing status) 1 is single and 2 is married, filing jointly
  mstat = c(1, 2),
...
The only difference between `usincometaxes` and TAXSIM 35 is that `usincometaxes` allows users to specify the state
with either the two letter abbreviation or SOI code. `usincometaxes` will convert the abbreviation to an SOI code for TAXSIM 35.

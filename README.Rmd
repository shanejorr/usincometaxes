...
The `interface` parameter to `taxsim_calculate_taxes()` controls how the TAXSIM 35 program is accessed. It has three options: (1) 'wasm', (2) 'ssh', and (3) 'http'.  With 'wasm', all tax calculations are done locally on one's own computer through [WebAssembly / JavaScript files](https://github.com/tmm1/taxsim.js) that are bundled with the package. With 'ssh' and 'http', the date are uploaded to the NBER's servers where the calculations are conducted. See the [TAXSIM input vignette](https://www.shaneorr.io/r/usincometaxes/articles/taxsim-input.html) for more information on the interface options.
